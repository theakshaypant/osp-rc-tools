...
Compare each index snapshot's operator revision against HEAD.

**Collect links:** Report snapshot names and creation times. Note any stale snapshots.

**Expected when DONE:** Bundle snapshot revision matches operator HEAD. All index snapshots are current.
